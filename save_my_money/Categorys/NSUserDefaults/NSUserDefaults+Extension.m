...
// 删除记账
+ (void)removeBookModel:(RecordModel *)model {
    // 删除
    NSMutableArray<RecordModel *> *bookArrm = [NSUserDefaults objectForKey:PIN_BOOK];
    NSMutableArray<RecordModel *> *bookSyncedArrm = [NSUserDefaults objectForKey:PIN_BOOK_SYNCED];
...
// 添加记账
+ (void)insertBookModel:(RecordModel *)model {
...
// 修改记账
+ (void)replaceBookModel:(RecordModel *)model {
    NSMutableArray *bookArr = [NSUserDefaults objectForKey:PIN_BOOK];
    NSMutableArray *bookSyncedArr = [NSUserDefaults objectForKey:PIN_BOOK_SYNCED];
...
+ (void)removeCategoryModel:(BKCModel *)model is_income:(BOOL)is_income {
    ...
    if (model.is_system == true) {
        if (is_income == false) {
            ...
            NSMutableArray<RecordModel *> *book = [NSUserDefaults objectForKey:PIN_BOOK];
            NSMutableArray<RecordModel *> *book_synced = [NSUserDefaults objectForKey:PIN_BOOK_SYNCED];
    ...
    // 删除同类别信息
    NSMutableArray<RecordModel *> *arrm = [NSUserDefaults objectForKey:PIN_BOOK];
    NSString *preStr = [NSString stringWithFormat:@"cmodel.Id == %ld", model.Id];
